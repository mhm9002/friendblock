...
if(isset($_POST['action'])){
    if($_POST['action'] == 'get-users'){
        $users = crPrivateMessenger::searchNonBlockedUsers($userID, $_POST['term']);
        $result = [];

        foreach($users as $row){
            $result[] = ["id" => $row['id'], 'label' => $row['name'], 'value' => $row['name'], 'hash' => cr_encrypt_id($row['id'])];
        }
    ...
    if($_POST['action'] == 'load-messenger'){
        echo loadMessenger();
        crUser::updateUserFields($userID, ['show_messenger' => 1]);
        cr_exit();
    }

    if($_POST['action'] == 'close-messenger'){
        crPrivateMessenger::closeConversationBox();
        crUser::updateUserFields($userID, ['show_messenger' => 0]);
        cr_exit();
    }
